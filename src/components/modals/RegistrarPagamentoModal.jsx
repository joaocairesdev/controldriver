...
export default function RegistrarPagamentoModal({
  ...
  async function buscarOuCriarFatura(cartao, dataBase) {
    ...
    const dataFechamento = dataComDiaSeguro(competencia.anoFechamento, competencia.mesFechamento, cartao.dia_fechamento);
            ...
            {!isCredito ? (
              ...
              <Campo label="Cartão utilizado">
                <ButtonField onClick={() => setModalCartaoAberto(true)}>
                  {cartaoSelecionado ? `${cartaoSelecionado.nome}${cartaoSelecionado.final_cartao ? ` final ${cartaoSelecionado.final_cartao}` : ""}` : "Selecionar cartão"}
                </ButtonField>
              </Campo>
            )}
          </div>
